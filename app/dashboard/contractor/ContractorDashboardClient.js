...
import {
    ArrowLeft, Building2, MapPin, Briefcase, Star,
    CheckCircle2, Clock, XCircle, ChevronRight,
    IndianRupee, Users, ClipboardList, PlusCircle,
    Lock, Sparkles, FileText, Wrench, UserRoundCheck, UserRoundX,
} from "lucide-react";
    ...
    if (status === "CLOSED") {
        return (
            <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-100 px-2.5 py-0.5 text-[11px] font-medium text-slate-600">
                <Lock className="h-3 w-3" /> {t("status.closed")}
            </span>
        );
    }
    return (
        <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-[11px] font-medium text-emerald-700">
            <CheckCircle2 className="h-3 w-3" /> {t("status.open")}
        </span>
    );
}

function ProfileProgress({ profile, t }) {
    const pct = profile?.companyName ? 100 : 0;
    return (
        <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs">
                <span className="font-medium text-slate-600">{t("contractor.profile.completion")}</span>
...
export default function ContractorDashboardClient({
    ...
    upsertAction, createJobAction, updateAppStatusAction, closeJobAction, reviewAction,
}) {
                ...
                <div className="relative mx-auto max-w-5xl px-4 sm:px-6">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            {/* Language toggle inside dashboard */}
                    ...
                    <div className="mt-6 sm:mt-8">
                        <p className="text-xs font-medium uppercase tracking-wider text-emerald-200">{t("contractor.welcomeBack")}</p>
                        <h1 className="mt-1 text-2xl font-bold text-white sm:text-3xl">{user.name}</h1>
                    </div>

                    <div className="mt-5 grid grid-cols-3 gap-3 sm:gap-4">
                        {[
                            { icon: <Building2 className="h-4 w-4" />, label: t("contractor.company"),     value: profile?.companyName ?? t("common.notSet") },
                            { icon: <Briefcase className="h-4 w-4" />, label: t("contractor.jobsPosted"),  value: jobs.length },
                            { icon: <Users className="h-4 w-4" />,     label: t("contractor.applicants"),  value: totalApplicants },
                        ].map(s => (
            ...
            <main className="relative z-10 mx-auto -mt-20 max-w-5xl space-y-8 px-4 pb-16 sm:-mt-24 sm:px-6">
                    ...
                    <Card className="border-0 shadow-lg shadow-slate-200/60">
                        <CardHeader className="border-b-0 pb-2">
                            <div className="flex items-center gap-2">
                                ...
                                <CardTitle className="text-base">{t("contractor.profile.title")}</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <ProfileProgress profile={profile} t={t} />
                            <Separator className="bg-slate-100" />
                            <form action={upsertAction} className="space-y-3">
                                <div className="space-y-1">
                                    <label className="flex items-center gap-1.5 text-xs font-medium text-slate-600" htmlFor="companyName">
                                        <Building2 className="h-3 w-3 text-slate-400" />
                                        {t("contractor.profile.companyName")}
                                    </label>
                                    <input id="companyName" name="companyName" defaultValue={profile?.companyName ?? ""} required
                                        className={inputClass} placeholder={t("contractor.profile.companyPlaceholder")} />
                                </div>
                                <button type="submit"
                                    ...
                                    {t("contractor.profile.save")}
                                </button>
                                <p className="text-[11px] text-slate-400">
                                    {profile ? t("contractor.profile.saved") : t("contractor.profile.fillOnce")}
                                </p>
                            </form>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-0 shadow-lg shadow-slate-200/60">
                        <CardHeader className="border-b-0 pb-2">
                            <div className="flex items-center gap-2">
                                ...
                                <CardTitle className="text-base">{t("contractor.postJob.title")}</CardTitle>
                            </div>
                        </CardHeader>
                        <CardContent>
                            <form action={createJobAction} className="space-y-3">
                                <div className="space-y-1">
                                    <label className="flex items-center gap-1.5 text-xs font-medium text-slate-600" htmlFor="title">
                                        ...
                                        {t("contractor.postJob.jobTitle")}
                                    </label>
                                    <input id="title" name="title" required className={inputClass} placeholder={t("contractor.postJob.jobTitlePlaceholder")} />
                                </div>
                                <div className="space-y-1">
                                    <label className="flex items-center gap-1.5 text-xs font-medium text-slate-600" htmlFor="description">
                                        <FileText className="h-3 w-3 text-slate-400" />
                                        {t("contractor.postJob.description")}
                                    </label>
                                    <textarea id="description" name="description" required rows={3}
                                        className={inputClass} placeholder={t("contractor.postJob.descriptionPlaceholder")} />
                                </div>
                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                                    <div className="space-y-1">
                                        <label className="flex items-center gap-1.5 text-xs font-medium text-slate-600" htmlFor="skillRequired">
                                            <Wrench className="h-3 w-3 text-slate-400" />
                                            {t("contractor.postJob.skillRequired")}
                                        </label>
                                        <input id="skillRequired" name="skillRequired" required className={inputClass} placeholder={t("contractor.postJob.skillPlaceholder")} />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="flex items-center gap-1.5 text-xs font-medium text-slate-600" htmlFor="city">
                                            <MapPin className="h-3 w-3 text-slate-400" />
                                            {t("contractor.postJob.city")}
                                        </label>
                                        <input id="city" name="city" defaultValue={user.city ?? ""} required className={inputClass} placeholder={t("contractor.postJob.cityPlaceholder")} />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="flex items-center gap-1.5 text-xs font-medium text-slate-600" htmlFor="wage">
                                            <IndianRupee className="h-3 w-3 text-slate-400" />
                                            {t("contractor.postJob.dailyWage")}
                                        </label>
                                        <input id="wage" name="wage" type="number" min={1} required className={inputClass} placeholder={t("contractor.postJob.wagePlaceholder")} />
                                    </div>
                                </div>
                                <button type="submit"
                                    ...
                                    {t("contractor.postJob.submit")}
                                </button>
                                <p className="text-[11px] text-slate-400">{t("contractor.postJob.note")}</p>
                            </form>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                    <Card className="border-0 shadow-md shadow-slate-200/60">
                        <CardContent className="flex flex-col items-center py-5 text-center">
                            ...
                            <p className="mt-0.5 text-xs text-slate-500">{t("contractor.overview.openJobs")}</p>
                        </CardContent>
                    </Card>
                    <Card className="border-0 shadow-md shadow-slate-200/60">
                        <CardContent className="flex flex-col items-center py-5 text-center">
                            <p className="text-2xl font-bold text-slate-600">{closedJobs.length}</p>
                            <p className="mt-0.5 text-xs text-slate-500">{t("contractor.overview.closedJobs")}</p>
                        </CardContent>
                    </Card>
                    <Card className="border-0 shadow-md shadow-slate-200/60">
                        <CardContent className="flex flex-col items-center py-5 text-center">
                            <p className="text-2xl font-bold text-violet-600">{totalApplicants}</p>
                            <p className="mt-0.5 text-xs text-slate-500">{t("contractor.overview.totalApplicants")}</p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <section className="space-y-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-semibold text-slate-900">{t("contractor.jobs.title")}</h2>
                        {jobs.length > 0 && <Badge variant="secondary" className="text-[10px]">{jobs.length}</Badge>}
                    </div>
                    <p className="text-sm text-slate-500">{t("contractor.jobs.desc")}</p>

                    {jobs.length === 0 ? (
                        <Card className="border-0 bg-slate-50/80 shadow-none">
                            <CardContent className="flex flex-col items-center justify-center py-10 text-center">
                                <Briefcase className="h-8 w-8 text-slate-300" />
                                <p className="mt-2 text-sm text-slate-400">{t("contractor.jobs.empty")}</p>
                            </CardContent>
                        </Card>
                    ) : (
                        <div className="space-y-5">
                            {jobs.map(job => (
                                <Card key={job.id} className="border-0 shadow-lg shadow-slate-200/60 transition-all hover:shadow-xl">
                                    <CardHeader className="border-b-0 pb-2">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                            <div className="space-y-1.5">
                                                ...
                                                <div className="flex flex-wrap gap-3 text-xs text-slate-500">
                                                    ...
                                                    <span className="inline-flex items-center gap-1"><Wrench className="h-3 w-3" />{job.skillRequired}</span>
                                                    <span className="inline-flex items-center gap-1"><IndianRupee className="h-3 w-3" />₹{job.wage}/day</span>
                                                </div>
                                            </div>
                                            {job.status === "OPEN" && (
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                <p className="text-xs font-semibold text-slate-700">
                                                    {t("contractor.jobs.applicants")} ({job.applications.length})
                                                </p>
                                            </div>

                                            {job.applications.length === 0 ? (
                                                <p className="mt-2 text-xs text-slate-400">{t("contractor.jobs.noApplications")}</p>
                                            ) : (
                                                <ul className="mt-3 space-y-2">
                                                    {job.applications.map(app => (
                                                        <li key={app.id} className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 transition-all hover:bg-slate-50">
                                                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                                <div className="space-y-0.5">
                                                                    <p className="text-sm font-medium text-slate-900">{app.labour.name}</p>
                                                                    <div className="flex flex-wrap gap-2 text-[11px] text-slate-500">
                                                                        <span className="inline-flex items-center gap-1">
                                                                            <MapPin className="h-2.5 w-2.5" />{app.labour.city}
                                                                        </span>
                                                            ...
                                                            {/* Review for closed accepted workers */}
                                                            {app.status === "ACCEPTED" && job.status === "CLOSED" && (() => {
                                                                const existing = job.reviews.find(r => r.revieweeId === app.labour.id);
                                                                return existing ? (
                                                                    <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700">
                                                                        ...
                                                                        {t("contractor.jobs.reviewed", { rating: existing.rating })}
                                                                    </div>
                                                                ) : (
                                                                    <form action={reviewAction} className="mt-3 space-y-2 rounded-lg border border-slate-100 bg-white p-3">
                                                                        <input type="hidden" name="jobId" value={job.id} />
                                                                        <input type="hidden" name="revieweeId" value={app.labour.id} />
                                                                        <p className="text-[11px] font-medium text-slate-600">{t("contractor.jobs.rateWorker")}</p>
                                                                        ...
                                                                        <textarea name="comment" rows={2} placeholder={t("contractor.jobs.optionalComment")}
                                                                        ...
                                                                        <button type="submit"
                                                                            ...
                                                                            {t("contractor.jobs.submitReview")}
                                                                        </button>
                                                                    </form>
                                                                );
                                                            })()}
